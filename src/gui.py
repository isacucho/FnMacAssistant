...
from .config import VERSION, FORTNITE_APP_PATH
...
class FnMacAssistantApp:
    ...
    def _apply_ttk_style(self):
        ...
        self.title_font.configure(
            size=int(
                default_font.cget("size") * FONT_SCALE_TITLE
            )
        )
    ...
    def _create_section(
        ...
        content_frame = ttk.Labelframe(
            parent, text="", padding=PAD_SECTION_INNER)
    ...
    def setup_ui(self):
        ...
        # --- Section 1: Download ---
        ipa_frame = self._create_section(
            main_frame, "⬇️ Download", info_command=self.show_ipa_info)
        ...
        if refresh_icon:
            ...
            refresh_btn = ttk.Button(
                ...
                command=self.refresh_dropdown)
        else:
            refresh_btn = ttk.Button(
                ipa_row1, text="↻", command=self.refresh_dropdown)
        refresh_btn.pack(side="right", padx=(GAP_S, 0))

        self.ipa_combobox = ttk.Combobox(ipa_row1, state="readonly")
        self.ipa_combobox.pack(
            side="left",
            fill="x",
            expand=True,
            padx=(
        ...
        self.copy_link_button = ttk.Button(
            ...
            style="Info.TButton")
        self.copy_link_button.pack(side="left", padx=(0, GAP_S))

        self.download_button = ttk.Button(
            download_row,
            text="Download Selected IPA",
        ...
        ttk.Button(
            download_row,
            text="Patch App",
            command=self.patch_app).pack(
            side="left",
            fill="x",
            expand=True)

        # --- Section 2: Game Data ---
        data_frame = self._create_section(
            main_frame,
            "📦 Game Data",
            refresh_command=self.detect_current_path)
        ...
        ttk.Button(
            data_row1,
            text="Open",
            command=self.open_fortnite_directory).pack(
            side="right")

        self.change_data_folder_button = ttk.Button(
            data_row1, text="Change Location", command=self.change_data_folder)
        ...
        ttk.Button(
            archive_row,
            text="Import Archive",
            command=self.import_archive).pack(
            side="left",
            padx=(
                GAP_S,
                0))
        ttk.Button(
            archive_row,
            text="🔗",
            command=self.show_archive_info).pack(
            side="left")
        ...
        self.cancel_download_button = ttk.Button(
            status_row, text="✖️", command=self.cancel_download)
    ...
    def detect_current_path(self):
        ...
        data_dir = self.app_manager.get_container_data_path(
        ...
        if self.app_manager.is_using_symlink(self.current_container):
            # Show the custom target path
            target = self.app_manager.get_container_data_display_path(
                self.current_container)
            # Shorten if too long
            if len(target) > 50:
                target = "..." + target[-47:]
            self.path_var.set(f"External: {target}")
        else:
            # Standard display
            self.path_var.set(self.current_container)

        if hasattr(self, "change_data_folder_button"):
            self.change_data_folder_button.state(["!disabled"])
            # Update button text based on whether we are using a symlink
            if self.app_manager.is_using_symlink(self.current_container):
                self.change_data_folder_button.config(
                    text="Reset Location", command=self.reset_data_folder)
            else:
                self.change_data_folder_button.config(
                    text="Change Location", command=self.change_data_folder)

    def get_container_root(self):
        # Always prefer the current container if valid
        ...
        if not containers:
            messagebox.showerror(
                "Error", "Could not find Fortnite's container.")
            return None
    ...
    def ask_user_to_choose_container_root(self, containers):
        selector = ContainerSelectorWindow(
            self.root, containers, self.app_manager)
        return selector.show()

    def change_data_folder(self):
        # Do not allow switching if we can't locate the current Game Data
        # folder
        container_root = self.get_container_root()
        if not container_root:
            messagebox.showerror(
                "Error", "Fortnite game data folder not found.")
            return

        current_data_dir = self.app_manager.get_container_data_path(
            container_root)
        ...
        folder_selected = filedialog.askdirectory(
            title="Select New Game Data Folder")
        ...
        def worker():
            try:
                result = self.app_manager.switch_data_folder(
                    container_root, folder_selected)
                self.root.after(
                    0, lambda: self._on_switch_data_folder_success(result))
            except Exception as e:
                self.root.after(
                    0, lambda err=e: self._on_switch_data_folder_error(err))
    ...
    def _on_switch_data_folder_success(self, result: dict):
        ...
        if status == "already":
            messagebox.showinfo(
                "Game Data Folder",
                "Selected folder is already in use.")
            return
    ...
    def _on_switch_data_folder_error(self, error: Exception):
        ...
        if isinstance(error, (PermissionError, OSError)
            ...
            messagebox.showerror(
                ...
                "Operation failed due to missing permissions. Please grant Full Disk Access.")
            return
        messagebox.showerror("Error", str(error))

    def reset_data_folder(self):
        container_root = self.current_container
        if not container_root:
            container_root = self.get_container_root()
        ...
        if not messagebox.askyesno(
            "Reset Location",
            "Are you sure you want to move the game data back to its original location?",
                icon='warning'):
            return

        self.update_status("Resetting data folder...")
        self._show_progress(True, "Resetting data folder...")

        def worker():
            try:
                # Use reset_data_location which returns True/False
                if self.app_manager.reset_data_location(container_root):
                    status = "restored"
                else:
                    status = "not_linked"
                self.root.after(
                    0, lambda: self._on_reset_data_folder_success(status))
            except Exception as e:
                self.root.after(
                    0, lambda err=e: self._on_reset_data_folder_error(err))

        threading.Thread(target=worker, daemon=True).start()

    def _on_reset_data_folder_success(self, status):
        self._show_progress(False)
        self.detect_current_path()
        if status == "not_linked":
            messagebox.showinfo(
                "Info", "Data is already at original location.")
        elif status == "restored":
            messagebox.showinfo(
                "Success", "Data moved back to original location.")
        elif status == "restored_empty":
            messagebox.showinfo(
                "Warning",
                "Symlink removed, but data target was missing. Created empty folder.")

    def _on_reset_data_folder_error(self, error):
        self._show_progress(False)
        self.update_status("Ready")
        messagebox.showerror("Error", f"Failed to reset: {str(error)}")
    ...
    def show_update_dialog(self, latest_version):
        ...
        ttk.Button(
            ...
            text="Update Now",
            command=lambda: [
                webbrowser.open("https://github.com/isacucho/FnMacAssistant/releases/latest"),
                update_window.destroy()]).pack(
            side=tk.LEFT,
            padx=10)
    ...
    def _fetch_ipa_data(self, force=False):
        ...
        if ipa_files:
            ...
            self.root.after(
                0, self._update_dropdown, list(
                    self.ipa_files_dict.keys()))
        else:
    ...
    def show_ipa_info(self):
        if messagebox.askyesno(
            "IPAs info",
            "For details on the differences between the IPAs, you can visit the project page.\n\nDo you want to open it in your browser?",
                icon="info"):
            webbrowser.open(
                "https://github.com/isacucho/FnMacAssistant#what-is-the-difference-between-the-various-ipas")
    ...
    def copy_download_link(self):
        ...
        if selected_ipa and selected_ipa in self.ipa_files_dict:
            url = self.ipa_files_dict[selected_ipa]['browser_download_url']
            self.root.clipboard_append(url)
            messagebox.showinfo("Link Copied",
                                f"Download link copied to clipboard:\n{url}")
        else:
    ...
    def update_download_progress(self, downloaded, total):
        if total > 0:
            ...
            self.progress_label.config(
                text=f"Downloaded {downloaded / (1024 * 1024):.2f} MB of {total / (1024 * 1024):.2f} MB")
        else:
            self.progress_label.config(
                text=f"Downloaded {downloaded / (1024 * 1024):.2f} MB")
    ...
    def patch_complete(self):
        ...
        messagebox.showinfo("Patch Complete",
                            "Fortnite has been patched. You can now open it.")
    ...
    def patch_error(self, error):
        ...
        if isinstance(error, (PermissionError, OSError)
            ...
            messagebox.showerror(
                "Permissions Required",
                "Failed to patch due to missing permissions. Please grant Full Disk Access.")
        else:
    ...
    def open_fortnite_directory(self):
        container_root_path = self.current_container
        if not container_root_path:
            container_root_path = self.get_container_root()

        if container_root_path:
            fortnite_downloaded_data_path = self.app_manager.get_fortnite_downloaded_data_path(
                container_root_path)
            subprocess.run(['open', '-R', fortnite_downloaded_data_path])

    def confirm_delete(self):
        container_path = self.current_container
        if not container_path:
            container_path = self.get_container_root()
        ...
        is_symlink = self.app_manager.is_using_symlink(container_path)
        if is_symlink:
            target = self.app_manager.get_container_data_display_path(
                container_path)
            warning_msg += (
                f"\n\nNOTICE: The custom data location at\n{target}\n"
                "will not be deleted, you can delete it manually"
            )
        ...
        if messagebox.askyesno(
            ...
            try:
                if self.app_manager.delete_app_and_data(container_path):
                    msg = "Fortnite app and container have been deleted."
                    if is_symlink:
                        msg += "\n\nRemember to manually delete the external data folder."
    ...
    def import_archive(self):
        ...
        def on_zip():
            ...
            if file_path:
                self.start_import_zip(file_path, fortnite_downloaded_data_path)
        ...
        def on_folder():
            ...
            if dir_path:
                if os.path.basename(dir_path) != "PersistentDownloadDir":
                    messagebox.showerror(
                        "Error", "Selected folder must be named 'PersistentDownloadDir'")
                    return
                self.start_import_folder(
                    dir_path, fortnite_downloaded_data_path)
    ...
    def start_import_folder(self, folder_path, target_dir):
        ...
        threading.Thread(
            ...
            kwargs={
                'progress_callback': self.update_import_progress,
                'completion_callback': self.import_complete,
                'error_callback': self.operation_error}).start()
